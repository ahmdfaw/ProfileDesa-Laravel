...
@section('title', 'Edit Transaksi Keuangan PAM')

@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Edit Transaksi Keuangan PAM</h1>
    </div>

    <div class="rounded-lg bg-white p-6 shadow">
        <form action="{{ route('admin.pam.keuangan.update', $pamKeuangan->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div class="mb-4">
                    ...
                    <input type="date" name="tanggal" id="tanggal"
                        value="{{ old('tanggal', $pamKeuangan->tanggal->toDateString()) }}"
                ...
                <div class="mb-4">
                    ...
                    <select name="jenis" id="jenis"
                        ...
                        required>
                        <option value="pemasukan" {{ old('jenis', $pamKeuangan->jenis) === 'pemasukan' ? 'selected' : '' }}>
                            Pemasukan</option>
                        <option value="pengeluaran"
                            {{ old('jenis', $pamKeuangan->jenis) === 'pengeluaran' ? 'selected' : '' }}>Pengeluaran</option>
                    </select>
                    @error('jenis')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
            ...
            <div class="mb-4">
                ...
                <input type="text" name="kategori" id="kategori" value="{{ old('kategori', $pamKeuangan->kategori) }}"
            ...
            <div class="mb-4">
                ...
                <textarea name="keterangan" id="keterangan" rows="3"
                    ...
                    required>{{ old('keterangan', $pamKeuangan->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah', $pamKeuangan->jumlah) }}"
                    min="0" step="100"
